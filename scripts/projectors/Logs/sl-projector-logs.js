...
async function fetchErrorData() {
    try {
        const [errorCover, errorFan, errorFilter, errorLamp, errorTemp] = await Promise.all([
            fetch(`${SL_BASE_URL}/${SL_VARIABLES.errorCover}/value`).then(r => r.text()),
            fetch(`${SL_BASE_URL}/${SL_VARIABLES.errorFan}/value`).then(r => r.text()),
            fetch(`${SL_BASE_URL}/${SL_VARIABLES.errorFilter}/value`).then(r => r.text()),
            fetch(`${SL_BASE_URL}/${SL_VARIABLES.errorLamp}/value`).then(r => r.text()),
            fetch(`${SL_BASE_URL}/${SL_VARIABLES.errorTemp}/value`).then(r => r.text())
        ]);
